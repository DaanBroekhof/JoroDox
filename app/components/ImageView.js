...
export default class ImageView extends Component {
    ...
    getBmpInfo(path) {
        let stream = jetpack.createReadStream(this.props.file.path, {end: 100});
        stream.on('readable', () => {
            let buf = stream.read(100);
            if (!buf) {
                stream.close();
                return;
            }
        ...
        return (
            <div style={{display: 'flex', flexDirection: 'column'}}>
               ...
               <div style={{
                ...
                }}>{imgSrc && <img src={imgSrc} style={{display: 'inline-block', margin: 0, objectFit: 'contain', maxWidth: '100%', maxHeight: '100%', width: 'auto', height: 'auto'}}/>}</div>
            </div>
        );
    }
}
