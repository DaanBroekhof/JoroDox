...
export default class PdxMesh {
  static convertToThreeJsScene(pdxData, path) {
    ...
    for (let i = 0; i < pdxData.props.object.children.length; i++) {
      if (pdxData.props.object.children[i].type !== 'object') { continue; }

      const pdxShape = pdxData.props.object.children[i];
      ...
      if ('skeleton' in pdxShape.props) {
        ...
        for (let j = 0; j < skeleton.children.length; j++) {
          ...
          const pdxBone = skeleton.children[j].props;
      ...
      for (let i = 0; i < pdxData.props.locator.children.length; i++) {
        const locator = pdxData.props.locator.children[i];
        ...
        const indicatorBone = new THREE.Bone();
      ...
      if (scene.bones) {
        const skeletonHelper = new THREE.SkeletonHelper(scene.bones[0]);
        for (let k = 0; k < skeletonHelper.geometry.attributes.color.array.length; k += 6) {
          const isLocator = !scene.bones[(k / 6)];
      ...
      for (let j = 0; j < pdxShape.children.length; j++) {
        if (pdxShape.children[j].type !== 'object') { continue; }

        const pdxMesh = pdxShape.children[j].props;
    ...
    if (rootLocatorBone) {
      const skeletonHelper = new THREE.SkeletonHelper(rootLocatorBone);
      for (let k = 0; k < skeletonHelper.geometry.attributes.color.array.length; k += 6) {
  ...
  static setPdxAnimation(scene, pdxAnimationData) {
    ...
    if (pdxAnimationData) {
      ...
      for (let k = 0; k < pdxAnimationData.props.info.children.length; k++) {
        const pdxAnimBone = pdxAnimationData.props.info.children[k];

        if (pdxAnimBone.type !== 'object') { continue; }

        let bone = null;
        // Assign 'base' animation state
        if (scene.object.bonesByName[pdxAnimBone.name]) { bone = scene.object.bonesByName[pdxAnimBone.name]; }
        if (!bone && alternativeNames[pdxAnimBone.name] && scene.object.bonesByName[alternativeNames[pdxAnimBone.name]]) { bone = scene.object.bonesByName[alternativeNames[pdxAnimBone.name]]; }
    ...
    /*
        ...
        */
    if (scene.animationMixer) { scene.animationMixer.stopAllAction(); }

    // 'Reset' skeleton and start new animation (if set)
    const subSkinnedMeshes = [];
    scene.object.traverse((subObject) => {
    ...
    if (animationData) {
      const animationClip = THREE.AnimationClip.parseAnimation(animationData, scene.object.bones);
      // scene.object.animations.push(animationClip);

      const mixer = new THREE.AnimationMixer(new THREE.AnimationObjectGroup(...subSkinnedMeshes));
  ...
  createFromThreeJsObject(object, options) {
    ...
    object.traverse((subObject) => {
      if (subObject instanceof THREE.Mesh) {
        ...
        const getVertexHash = function (itemNr, attributes) {
          const arr = [];
        ...
        const vertexIndex = [];
        const reverseVertexIndex = [];
        const nrVerts = subObject.geometry.attributes.position.count;
        const indexLookup = new Map();
        for (let k = 0; k < nrVerts; k++) {
          const hash = getVertexHash(k, subObject.geometry.attributes);
        ...
        const getIndexedAttribute = function (attribute, reverseVertexIndex) {
          const data = [];
        ...
        const indexedPositions = getIndexedAttribute(subObject.geometry.attributes.position, reverseVertexIndex);
        const indexedNormals = getIndexedAttribute(subObject.geometry.attributes.normal, reverseVertexIndex);
        const indexedTangents = getIndexedAttribute(subObject.geometry.attributes.tangent, reverseVertexIndex);
        const indexedSkinIndexes = getIndexedAttribute(subObject.geometry.attributes.skinIndex, reverseVertexIndex);
        const indexedSkinWeights = getIndexedAttribute(subObject.geometry.attributes.skinWeight, reverseVertexIndex);
        const indexedUVs = [];
        if (subObject.geometry.attributes.uv) { indexedUVs.push(getIndexedAttribute(subObject.geometry.attributes.uv, reverseVertexIndex)); }
        if (subObject.geometry.attributes.uv1) { indexedUVs.push(getIndexedAttribute(subObject.geometry.attributes.uv1, reverseVertexIndex)); }
        if (subObject.geometry.attributes.uv2) { indexedUVs.push(getIndexedAttribute(subObject.geometry.attributes.uv2, reverseVertexIndex)); }
        ...
        for (let i = 0; i < indexedSkinWeights.length; i += 4) {
          ...
          const bonesUsedInVertex = Math.ceil(indexedSkinWeights[i]) + Math.ceil(indexedSkinWeights[i + 1]) + Math.ceil(indexedSkinWeights[i + 2]) + Math.ceil(indexedSkinWeights[i + 3]);
  ...
  convertToPdxAnim(animation, viewObject) {
    ...
    for (let i = 0; i < boneList.length; i++) {
      ...
      for (let k = 0; k < animation.tracks.length; k++) {
        const trackFps = animation.tracks[k].times.length / animation.tracks[k].times[animation.tracks[k].times.length - 1];
    ...
    for (let k = 0; k < animation.tracks.length; k++) {
      if (!animation.tracks[k].found) { console.log(`Could not find bone for track \`${animation.tracks[k].name}\`.`); }
    }

    const trackArray = [trackCount];
    const pdxInfo = {
      name: 'info',
      type: 'object',
      children: [
    ...
    for (let i = 0; i < tracksPerBone.length; i++) {
      ...
      if (tracksPerBone[i].position) {
        ...
        if (!boneData[i].hasTransformChange) { trackArray[0]--; }
      }
      if (tracksPerBone[i].quaternion) {
        ...
        if (!boneData[i].hasRotationChange) { trackArray[0]--; }
      }
      if (tracksPerBone[i].scale) {
        ...
        if (!boneData[i].hasScaleChange) { trackArray[0]--; }
      }
    ...
    for (let k = 0; k < sampleCount; k++) {
      ...
      for (let i = 0; i < tracksPerBone.length; i++) {
        ...
        if (boneData[i].hasScaleChange) {
          const scaleValues = tracksPerBone[i].scale.createInterpolant().evaluate(keyTime);
