...
export default class PdxYmlFileParserTask extends DbBackgroundTask {
    ...
    execute(args) {
        JdxDatabase.get(args.root).then(db => {
            ...
            db.files.where('path').startsWithAnyOf(prefixes).filter(file => _(patterns).some(pattern => minimatch(file.path, pattern))).toArray(files => {
                ...
                let relations = [];
                filesList.each(file => {
                    let filePath = args.root + syspath.sep + file.path.replace(new RegExp('/', 'g'), syspath.sep);
                    let fileData = jetpack.read(filePath);
                    let pdxYmlData = this.parsePdxYml(fileData, filePath);
    ...
    parsePdxYml(data, filePath) {
        ...
        data.split("\n").forEach((line, lineNr) => {
            ...
            let typeMatch = line.match(/^(.+):\s*(#\s*(.*))?$/u);
            if (typeMatch) {
                type = typeMatch[1];
                result[type] = {};
                return;
