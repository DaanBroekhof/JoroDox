export default class PdxData {
    ...
    readFromBuffer(buffer) {
        ...
        let base = {type: 'object', name: 'pdxData', children: [], depth: 0, 'props': {}};
    ...
    readObject(object, data, offset, objectDepth)
    {
        ...
        if (depth > 0)
        {
            let name = this.readNullByteString(data, offset);
            offset += name.length + 1;
            let newObject = {type: 'object', 'name': name, children: [], 'depth': depth, 'props': {}};
        ...
        while (offset < data.byteLength)
        {
            ...
            {
                let newOffset = this.readObject(object, data, offset, depth);
    ...
    readProperty(object, data, offset)
    {
        ...
        let propertyNameLength = data.getInt8(offset);
        offset++;
        // Propname
        let propertyName = this.readString(data, offset, propertyNameLength);
        offset += propertyNameLength;

        // Value
        let property = this.readRawData(data, offset);
    ...
    {
        let result = {data: null, offset: 0, type: null};
        ...
        else if (data.getInt8(offset) === 's'.charCodeAt(0))
        {
            ...
            offset += 4;
            let strLength = data.getUint32(offset, true);
            offset += 4;
    ...
    extendBuffer(buffer, extend)
    {
        ...
        {
            let tmp = new Uint8Array(new ArrayBuffer(buffer.current.byteLength + buffer.extendSize));
    ...
    writeObject(buffer, objectData)
    {
        ...
        let l = objectData.children.length;
        for (let i = 0; i < l; i++)
        {
    ...
    writeStringProperty(buffer, propertyData)
    {
        ...
        let l = propertyData.data.length + (propertyData.nullByteString ? 1 : 0);
        this.writeUint32(buffer, l);
